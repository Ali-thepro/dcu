...
for line in sys.stdin:
    ...
    for n in numbers:
        if int(n) in mapping:
            ...
            output += f"{mapping[int(n[0]) * 10]}-{mapping[int(n[1])]} "
    print(output.strip())
...
# for line in numbers:
#     # splits it according to white space
#     digits = line.split()
#     words = []
#     # now it takes into account every individual value
#     # split line -> then focus on the individual elements
#     for digit in digits:
#         # sets condition for output
#         if digit in number:
